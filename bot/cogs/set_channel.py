...
class SetChannelCommand(commands.Cog):
    ...
    async def sc(
        self,
        inter,
        genre: commands.option_enum(GENRES),
        ...
        cfg.setdefault("send_channel_ids", defaultcfg.CFG_DEFAULT["send_channel_ids"])
        cfg["send_channel_ids"][genre] = text_channel.id
        cfg.setdefault("forum_channel_ids", defaultcfg.CFG_DEFAULT["forum_channel_ids"])
