    ...
    async def dc(
        ...
        if str(inter.guild.id) in db.keys():
            ...
            if "send_channel_ids" in cfg:
                ...
                await inter.response.send_message(
                    "テキストチャンネルは登録されていません", ephemeral=True
                )
            if "forum_channel_ids" in cfg:
                if genre in cfg["forum_channel_ids"]:
                    ...
                    db[str(inter.guild_id)] = json.dumps(cfg)
                    await inter.response.send_message(
                        "フォーラムチャンネルの通知設定を解除しました"
                    )
                else:
                    await inter.response.send_message(
                        "対応するフォーラムチャンネルは登録されていません",
                        ephemeral=True,
                    )
            else:
                await inter.response.send_message(
                    "フォーラムチャンネルは登録されていません", ephemeral=True
                )
        else:
            await inter.response.send_message(
                "データベースにサーバーが登録されていません", ephemeral=True
            )
