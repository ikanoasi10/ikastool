...
class TableturfCommand(commands.Cog):
    def __init__(self, bot: commands.Bot):
        self.bot = bot

    @commands.slash_command(name="ナワバトラー")
    async def any_spla3(
        self,
        ...
        at: str = commands.Param(
            default="",
            name="at",
            choices=([str(i) for i in reversed(range(1, 7 + 1))])),
        max: str = commands.Param(
            default="",
            name="max",
...
def setup(bot: commands.Bot):
    bot.add_cog(TableturfCommand(bot))
