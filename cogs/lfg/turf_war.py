...
class TurfWarCommand(commands.Cog):
    def __init__(self, bot: commands.Bot):
        self.bot = bot

    @commands.slash_command(name="ナワバリ")
    async def turf_war(self,
                inter: disnake.ApplicationCommandInteraction,
                at: str = commands.Param(default="", name="at", choices=([str(i) for i in reversed(range(1, 7 + 1))])),
                max: str = commands.Param(default="",name="max", choices=([str(i) for i in reversed(range(1, 7 + 1))])),
...
def setup(bot: commands.Bot):
    bot.add_cog(TurfWarCommand(bot))
