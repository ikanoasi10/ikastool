...
# Start
1. .env `TOKEN`にDiscord botのTokenを入れておく(Discord Developper Portalで取得)
2. `docker build -t ikastool-py . && docker run -d ikastool-py`
3. botを招待する
4. botが動く
5. `/へルプ` を見てください
...
❌で削除、1️⃣～9️⃣で@の人数変更、🔚で募集を〆ることができます
